    ...
    switch(action.type) {
        case CART_ADD_ITEM:
            if (state.items.some(({id}) => id === action.payload)) {
                return {...state, items: state.items.map(({id, count}) => 
                    (id === action.payload) ? {id: id, count: count+1} : {id: id, count: count}
                )};
            }
            else return {...state, items: [...state.items, {id: action.payload, count: 1}] };

        case CART_REMOVE_ITEM:
            let newItems = state.items.map(({id, count}) => 
                (id === action.payload) ? {id: id, count: count-1} : {id: id, count: count}    
            );
            return {...state, items: newItems.filter(({count}) => count !== 0) };
